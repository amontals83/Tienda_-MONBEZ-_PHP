...
class Clientes extends Controllers
{
	...
	public function setCliente()
	{
		...
		if($_POST)
		{
			if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['txtDNI']) || empty($_POST['txtNombreFiscal']) || empty($_POST['txtDirFiscal']) )
			{
	...
	public function getClientes()
	{
		if($_SESSION['permisosMod']['r'])
		{
			...
			for ($i=0; $i < count($arrData); $i++) 
			{
				...
				if($_SESSION['permisosMod']['r'])
				{
					$btnView = '<button class="btn btn-primary active btn-sm" onClick="fntViewInfo('.$arrData[$i]['idpersona'].')" title="Ver cliente"><i class="far fa-eye"></i></button>';
				}
				if($_SESSION['permisosMod']['u'])
				{
